...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('createQuizApp', ['ngRoute', 'ngAnimate']).config(['$routeProvider', function ($routeProvider) {
	    $routeProvider.when('/', {
	...
	angular.module('createQuizApp').factory('QuizData', ['$http', '$log', function ($http, $log) {
	    ...
	    function createUserId(callback) {
	        if (!userUuid) {
	            ...
	            $http.post('/create/profile', { uuid: userUuid }).success(function (result) {
	    ...
	    var postDelete = function postDelete(quiz) {
	        $log.debug(quiz);
	        return $http.post('/create/' + userUuid + '/quizzes/' + quiz.uuid + '/delete', quiz);
	    };
	
	    var postQuiz = function postQuiz(quiz) {
	        $log.debug(quiz);
	    ...
	    var getQuiz = function getQuiz(uuid) {
	    ...
	    var postTopic = function postTopic(topic) {
	        return $http.post('/create/' + userUuid + '/topics/', topic);
	    };
	
	    return {
	        getSampleQuizzes: function getSampleQuizzes(callback) {
	            callback(sampleData);
	        },
	        getQuizzes: (function (_getQuizzes) {
	            function getQuizzes(_x) {
	                return _getQuizzes.apply(this, arguments);
	            }
	
	            getQuizzes.toString = function () {
	                return _getQuizzes.toString();
	            };
	
	            return getQuizzes;
	        })(function (callback) {
	            if (userUuid != '') {
	                getQuizzes().success(function (resp) {
	                    $log.debug(resp);
	                    data = resp;
	                    callback(data);
	                });
	        ...
	        getTopics: (function (_getTopics) {
	            function getTopics(_x2) {
	                return _getTopics.apply(this, arguments);
	            }
	
	            getTopics.toString = function () {
	                return _getTopics.toString();
	            };
	        ...
	        getQuiz: (function (_getQuiz) {
	            function getQuiz(_x3, _x4, _x5) {
	                return _getQuiz.apply(this, arguments);
	            }
	
	            getQuiz.toString = function () {
	                return _getQuiz.toString();
	            };
	
	            return getQuiz;
	        })(function (id, loadFromServer, callback) {
	        ...
	        deleteQuiz: function deleteQuiz(id, callback) {
	            var quiz = data[id];
	            data.splice(id, 1);
	        ...
	        saveQuiz: function saveQuiz(id, quiz, topics) {
	            data[id] = quiz;
	        ...
	        addQuiz: function addQuiz(quiz, callback) {
	        ...
	        publishQuiz: function publishQuiz(quiz, details) {
	        ...
	        saveClassCode: function saveClassCode(newClassCode) {
	            if (newClassCode) localStorage.setItem('classCode', newClassCode);
	        },
	...
	angular.module('createQuizApp').controller('QuizzesController', ['QuizData', '$log', '$http', '$location', function (QuizData, $log, $http, $location) {
	    ...
	    if (self.token != undefined) {
	        $http.get('/quiz/token/' + self.token).success(function (result) {
	        ...
	        $http.get('/quiz/profile/' + self.userId).success(function (result) {
	            //to get user uuid and name
	            QuizData.setUser(result);
	            QuizData.getQuizzes(function (data) {
	                self.pastQuizzes = data;
	    ...
	    self.createQuiz = function () {
	        if (self.newQuizName.length == 0) self.newQuizName = 'Quiz';
	...
	angular.module('createQuizApp').controller('CreateController', ['QuizData', '$log', '$location', '$routeParams', '$timeout', function (QuizData, $log, $location, $routeParams, $timeout) {
	    ...
	    self.editQuestion = function (idx) {
	        ...
	        if (q.alternatives) {
	            for (var i = 1; i < 4; i++) $('#alt' + i).val(q.alternatives[i - 1]);
	        }
	        $('#question').focus();
...
/***/ }
/******/ ]);
//# sourceMappingURL=createQuizApp.js.map