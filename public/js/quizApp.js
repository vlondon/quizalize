...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('quizApp', ['ngRoute', 'ngAnimate']).config(['$routeProvider', function ($routeProvider) {
	    $routeProvider.when('/', {
	...
	angular.module('quizApp').factory('ZzishContent', ['$http', '$log', '$rootScope', function ($http, $log, $rootScope) {
	    ...
	    return {
	        ...
	        user: function user(id, name, code, callback) {
	            zzish.authUser(id, name, code, function (err, message) {
	                callback(err, message);
	            });
	        },
	        validate: function validate(classCode) {
	            self.code = classCode.toLowerCase();
	            return zzish.validateClassCode(classCode);
	        },
	        getPublicQuizzes: function getPublicQuizzes(callback) {
	        ...
	        register: function register(profileId, classCode, callback) {
	            self.code = classCode.toLowerCase();
	        ...
	        login: function login(studentCode, callback) {
	        ...
	        logout: function logout(userProfileId, callback) {
	        ...
	        getConsumerContent: function getConsumerContent(contentId, callback) {
	            zzish.getConsumerContent(self.userId, contentId, function (err, message) {
	                callback(err, message);
	                $rootScope.$apply();
	            });
	        },
	        startActivity: function startActivity(quiz, callback) {
	            var parameters = {
	                activityDefinition: {
	                    type: quiz.uuid,
	                    name: quiz.name,
	...
	angular.module('quizApp').factory('QuizData', ['$http', '$log', '$location', 'ZzishContent', function ($http, $log, $location, ZzishContent) {
	    ...
	    var chooseKind = function chooseKind(qId) {
	        var randomKind = function randomKind() {
	            var kinds = ['scrambled'];
	            return kinds[Math.floor(Math.random() * kinds.length)];
	        };
	
	        var isIntegerAnswer = function isIntegerAnswer(ans) {
	            return !!ans.match(/^-?\d+$/);
	        };
	
	        var generateIntegerAlternatives = function generateIntegerAlternatives(ans) {
	            var ai = parseInt(ans);
	        ...
	        if (typeof qId != 'undefined') {
	            ...
	            if (question && isIntegerAnswer(question.answer)) {
	                if (!question.alternatives) question.alternatives = generateIntegerAlternatives(question.answer);
	                return 'multiple';
	            }
	     ...
	     */
	    var processQuizList = function processQuizList(result, isClassCode) {
	        if (isClassCode) {
	    ...
	    var calculateScore = function calculateScore(correct, duration) {
	    ...
	    return {
	        login: function login(newStudentCode, callback) {
	        ...
	        validate: function validate(newClassCode) {
	            return ZzishContent.validate(newClassCode);
	        },
	        register: function register(studentName, newClassCode, callback) {
	        ...
	        selectQuiz: function selectQuiz(categoryId, quizId, callback) {
	            var found = false;
	        ...
	        getQuestion: function getQuestion(id, questionCallback) {
	        ...
	        chooseKind: (function (_chooseKind) {
	            function chooseKind(_x) {
	                return _chooseKind.apply(this, arguments);
	            }
	
	            chooseKind.toString = function () {
	                return _chooseKind.toString();
	            };
	        ...
	        getAlternatives: function getAlternatives(id) {
	            ...
	            if (currentQuiz.questions[id].alternatives) {
	                ...
	                for (var i in currentQuiz.questions[id].alternatives) {
	                    ...
	                    if (alt != undefined && alt.length > 0) options.push(alt);
	                }
	        ...
	        answerQuestion: function answerQuestion(idx, response, answer, questionName, duration) {
	...
	angular.module('quizApp').controller('StartController', ['QuizData', '$log', '$location', '$rootScope', function (QuizData, $log, $location, $rootScope) {
	    ...
	    var reportError = function reportError(issue) {
	        self.loading = false;
	    ...
	    if (localStorage.getItem('zname') != undefined && localStorage.getItem('zname') != '' && localStorage.getItem('zcode') != undefined && localStorage.getItem('zcode') != '') {
	...
	angular.module('quizApp').controller('QuizController', ['QuizData', '$log', '$routeParams', '$location', function (QuizData, $log, $routeParmas, $location) {
	    ...
	    var getLetters = function getLetters(answer) {
	    ...
	    var getUserAnswer = function getUserAnswer(n) {
	        var ans = [];
	        for (var i = 0; i < n; i++) {
	            ans.push('_');
	        }
	    ...
	    var arrayMatch = function arrayMatch(a, b) {
	    ...
	    var replaceSpaces = function replaceSpaces(s) {
	    ...
	    var updateLastEmpty = function updateLastEmpty() {
	    ...
	    self.submit = function () {
	        if (self.userAnswerLetters[self.userAnswerLetters.length - 1] != '_') {
	            QuizData.answerQuestion(self.questionId, self.userAnswerLetters.join('').toUpperCase(), self.answer.toUpperCase(), self.question, new Date().getTime() - startTime);
	        } else {}
	    };
	...
	angular.module('quizApp').controller('MultipleController', ['QuizData', '$log', '$routeParams', '$location', function (QuizData, $log, $routeParmas, $location) {
	    ...
	    self.select = function (idx) {
	        QuizData.answerQuestion(self.questionId, self.alternatives[idx], self.answer, self.question, new Date().getTime() - startTime);
	    };
	...
	angular.module('quizApp').controller('ManualController', ['QuizData', '$log', '$routeParams', '$location', function (QuizData, $log, $routeParmas, $location) {
	    ...
	    self.manualAnswer = function () {
	        QuizData.answerQuestion(self.questionId, self.yourAnswer, self.answer, self.question, new Date().getTime() - startTime);
	    };
	...
	angular.module('quizApp').controller('CompleteController', ['QuizData', '$log', '$location', function (QuizData, $log) {
	    ...
	    var calculateTotals = function calculateTotals(items) {
